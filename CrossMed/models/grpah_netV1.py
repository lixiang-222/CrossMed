    ...
    def __init__(self, src_names, tgt_names, num_nodes_dict, dim):
        ...
        for src in src_names:
            for tgt in tgt_names:
                ...
                self.embeddings[str(key)] = nn.Parameter(torch.ones(num_src_nodes, num_tgt_nodes, dim))
        ...
        for tgt in tgt_names:
            for src in src_names:
                if src != tgt:  # 避免重复添加相同类型节点之间的关系
                    ...
                    num_tgt_nodes = num_nodes_dict[tgt]
                    num_src_nodes = num_nodes_dict[src]
                    self.embeddings[str(key)] = nn.Parameter(torch.ones(num_tgt_nodes, num_src_nodes, dim))
                    self.edge_types.append(key)
        ...
        # TODO：感觉这里有错，为什么没有同类节点直接的关系，但是不敢改，之后有机会需要改一下

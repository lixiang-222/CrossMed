...
class KAME(nn.Module):
    ...
    def forward(self, batchdata=None, **kwargs):
        ...
        patient_emb = torch.cat(patient_emb, dim=1)
        # print(f"Final patient_emb shape: {patient_emb.shape}")########
        # print(f"FC layer expects input size: {self.fc.in_features}")
        logits = self.fc(patient_emb)
        # return logits
        # 计算损失
        if 'drugs' in kwargs:
            # print(f"drugs type: {type(kwargs['drugs'])}")
            # print(f"drugs sample: {kwargs['drugs'][:2]}")  # 添加这两行
            drugs_tokenizer = self.feat_tokenizers['drugs_hist']
            # 处理未知token
            valid_drugs = []
            for patient_drugs in kwargs['drugs']:
                valid_patient_drugs = []
                for drug in patient_drugs:
                    if drug in drugs_tokenizer.vocabulary.token2idx:
                        valid_patient_drugs.append(drug)
                valid_drugs.append(valid_patient_drugs)

            y_indices = drugs_tokenizer.batch_encode_2d(valid_drugs)
            y_true = torch.zeros(len(y_indices), self.fc.out_features, device=self.device)
            
            for i, indices in enumerate(y_indices):
                y_true[i, indices] = 1.0
            criterion = torch.nn.BCEWithLogitsLoss()
            loss = criterion(logits, y_true)
            y_prob = torch.sigmoid(logits)  # 将logits转为概率
            return {"logits": logits, "loss": loss, "y_true": y_true, "y_prob": y_prob}
